...
class LiRPAConvNet:
    def __init__(
            ...
            cplex_processes=None, input = None, full_alpha = True):
        """
        ...
        if input is not None:
            self.net = BoundedModule(net, (input), 
                                     bound_opts={
                    'relu': 'adaptive',
                    ...
                    'use_full_conv_alpha' : full_alpha and arguments.Config["solver"]["alpha-crown"]["full_conv_alpha"],
                })
        else:
        ...
        #initialize cross-ex default params
        if arguments.Config["cross_ex"]["enable_cross_ex"]:
            self.init_refinement_params()
    
    def init_refinement_params(self, method = 'CROWN-Optimized',needed_A_dict = None, multiple_execution = False, execution_count = 1, ptb = None, unperturbed_images = None, iteration = None, baseline_refined_bound = {}, intermediate_bound_refinement = False, always_correct_cross_execution = False, cross_refinement_results = {}, populate_trace = False, index = None):
        self.method = method
        self.return_A = True
        self.needed_A_dict = needed_A_dict
        self.multiple_execution = multiple_execution
        self.execution_count = execution_count if execution_count is not None else 1
        self.ptb = ptb
        self.unperturbed_images = unperturbed_images
        self.iteration = iteration
        self.baseline_refined_bound = baseline_refined_bound
        self.intermediate_bound_refinement = intermediate_bound_refinement
        self.always_correct_cross_execution = always_correct_cross_execution
        self.cross_refinement_results = cross_refinement_results
    ...
    def get_lower_bound(self, pre_lbs, pre_ubs, split, slopes=None, betas=None, history=None, fix_intermediate_layer_bounds=True,
                        ...
                        stop_func=stop_criterion_sum(0), multi_spec_keep_func=None, ptb=None, topk=None):
        ...
        if arguments.Config["cross_ex"]["enable_cross_ex"]:
            lower_bounds, upper_bounds, lAs, slopes, betas, split_history, best_intermediate_betas, primals, new_cs, A = ret
        else:
        ...
        return (upper_bounds[-1], lower_bounds[-1], None, lAs, lower_bounds, \
               upper_bounds, slopes, split_history, betas, best_intermediate_betas, primals, new_cs, A) if arguments.Config["cross_ex"]["enable_cross_ex"] else (upper_bounds[-1], lower_bounds[-1], None, lAs, lower_bounds, \
    ...
    def reset_beta(self, batch, max_splits_per_layer=None, betas=None, diving_batch=0, device = True):
        device = self.net.device #if device else 'cpu'
        ...
        for mi, m in enumerate(self.net.perturbed_optimizable_activations):
            if isinstance(m, BoundRelu):
                ...
                if arguments.Config["solver"]["beta-crown"]['enable_opt_interm_bounds']:
                    ...
                    for key in m.sparse_beta.keys():
                        m.sparse_beta[key] = m.sparse_beta[key].repeat(2, 1).detach().to(device= device, non_blocking=True).requires_grad_()
                else:
                    ...
                    for bi in range(batch):
                        if betas is not None and betas[bi] is not None:
                            ...
                            try:
                                ...
                                raise RuntimeError
                    # This is the beta variable to be optimized for this layer.
                    m.sparse_beta = m.sparse_beta.repeat(2, 1).detach().to(device=device, non_blocking=True).requires_grad_()
                ...
                if diving_batch != 0:
                    ...
                    m.diving_sparse_beta = m.diving_sparse_beta.to(device=device, non_blocking=True)
                    m.sparse_beta = torch.cat([m.sparse_beta, m.diving_sparse_beta], dim=0).detach().\
                                to(device=device, non_blocking=True).requires_grad_()
    ...
    def update_bounds_parallel(
            ...
            multi_spec_keep_func=None, topk=None):
        ...
        if fix_intermediate_layer_bounds:
            ...
            if arguments.Config["cross_ex"]["enable_cross_ex"]:
                tmp_ret = self.net.compute_bounds(
                    x=(new_x,), C=c, method= self.method,
                    ...
                    always_correct_cross_execution = self.always_correct_cross_execution, cross_refinement_results = self.cross_refinement_results, populate_trace = self.populate_trace, topk=topk)
            else:
                tmp_ret = self.net.compute_bounds(
        ...
        return (ret_l, ret_u, lAs, ret_s, ret_b, new_split_history, best_intermediate_betas, primal_x, c, A) if arguments.Config["cross_ex"]["enable_cross_ex"] else (ret_l, ret_u, lAs, ret_s, ret_b, new_split_history, best_intermediate_betas, primal_x, c)
    ...
    def update_bounds_cut_naive(
            ...
            batchwise_out=True, cs=None, target_lAs= None, topk=None):
        ...
        if arguments.Config["cross_ex"]["enable_cross_ex"]:
            lb, _, A = self.net.compute_bounds(x=(new_x,), C=c, method= self.method,
                ...
                return_A= self.return_A, needed_A_dict = self.needed_A_dict, multiple_execution = self.multiple_execution, execution_count = self.execution_count, ptb = ptb, unperturbed_images = self.unperturbed_images, 
                    iteration = self.iteration, baseline_refined_bound = self.baseline_refined_bound, intermediate_bound_refinement = self.intermediate_bound_refinement,
                    always_correct_cross_execution = self.always_correct_cross_execution, cross_refinement_results = self.cross_refinement_results, populate_trace = self.populate_trace, target_lAs= target_lAs, topk=topk)
        else:
        ...
        with torch.no_grad():
            if not batchwise_out:
                ...
                return (ub[-1], lb[-1], None, None, None, mask[0], lA[0], lb, ub, None, slope_opt, history, new_split_history, A) if arguments.Config["cross_ex"]["enable_cross_ex"] else (ub[-1], lb[-1], None, None, None, mask[0], lA[0], lb, ub, None, slope_opt, history, new_split_history)
            else:
                # Move tensors to CPU for all elements in this batch.
                start_transfer_time = time.time()
                lb = lb.to(device='cpu')
        ...
        print(f'This batch time : update_bounds func: {func_time:.4f}\t prepare: {prepare_time:.4f}\t bound: {bound_time:.4f}\t transfer: {transfer_time:.4f}\t finalize: {finalize_time:.4f}')
        print(f'Accumulated time: update_bounds func: {total_func_time:.4f}\t prepare: {total_prepare_time:.4f}\t bound: {total_bound_time:.4f}\t transfer: {total_transfer_time:.4f}\t finalize: {total_finalize_time:.4f}')
        return ret_l, ret_u, lAs, ret_s, betas, new_split_history, best_intermediate_betas, None, c, A if arguments.Config["cross_ex"]["enable_cross_ex"] else ret_l, ret_u, lAs, ret_s, betas, new_split_history, best_intermediate_betas, None, c
    ...
    def build_the_model(self, input_domain, x, data_lb=None, data_ub=None, vnnlib=None, stop_criterion_func=stop_criterion_sum(0), bounding_method=None, target_lAs = None, target_biases = None, topk=None):
        """
        ...
        if arguments.Config["bab"]["cut"]["enabled"]:
            ...
            tmp1 = self.needed_A_dict.get(self.net.output_name[0], set())
            if isinstance(tmp1, set):
                tmp1.add(self.net.input_name[0])
            elif isinstance(tmp1, list):
                tmp1.append(self.net.input_name[0])
            else:
                raise RuntimeError
            for l in self.net.relus:
                tmp2 = self.needed_A_dict.get(l.inputs[0].name, set())
                if isinstance(tmp2, set):
                    tmp2.add(self.net.input_name[0])
                elif isinstance(tmp2, list):
                    tmp2.append(self.net.input_name[0])
                else:
                    raise RuntimeError
        ...
        if bounding_method == "alpha-crown":
            ...
            if arguments.Config["cross_ex"]["enable_cross_ex"]:
                lb, ub, aux_reference_bounds, A = self.net.init_slope(
                    (self.x,), share_slopes=share_slopes, c=self.c, bound_upper=False, return_A= self.return_A, needed_A_dict = self.needed_A_dict, multiple_execution = self.multiple_execution, execution_count = self.execution_count, ptb = self.ptb, unperturbed_images = self.unperturbed_images, iteration = self.iteration, baseline_refined_bound = self.baseline_refined_bound, intermediate_bound_refinement = self.intermediate_bound_refinement,
            ...
            if stop_criterion_func(lb).all().item():
                ...
                return (np.inf, lb, None, None, None, None, None, None, None, None, None, None, None, A) if arguments.Config["cross_ex"]["enable_cross_ex"] else (np.inf, lb, None, None, None, None, None, None, None, None, None, None, None)
            ...
            if arguments.Config["attack"]["pgd_order"] == "middle" and vnnlib is not None:
                ...
                if verified_success:  # Adversarial images are generated here.
                    ...
                    return (None, lb[-1], None, None, None, None, None, None, None, None, None, None, attack_images, A) if arguments.Config["cross_ex"]["enable_cross_ex"] else (None, lb[-1], None, None, None, None, None, None, None, None, None, None, attack_images)
            ...
            if arguments.Config["cross_ex"]["enable_cross_ex"]:
                ret = self.net.compute_bounds(x=(x,), C=c_to_use, method= self.method,
                    bound_upper=False, aux_reference_bounds=aux_reference_bounds, cutter=self.cutter, return_A = self.return_A,
                    needed_A_dict = self.needed_A_dict, multiple_execution = self.multiple_execution, execution_count = self.execution_count, ptb = self.ptb, unperturbed_images = self.unperturbed_images, 
                    iteration = self.iteration, baseline_refined_bound = self.baseline_refined_bound, intermediate_bound_refinement = self.intermediate_bound_refinement,
                    always_correct_cross_execution = self.always_correct_cross_execution, cross_refinement_results = self.cross_refinement_results, populate_trace = self.populate_trace, target_lAs= target_lAs, target_biases = target_biases, topk=topk)
            else:
            ...
            return (ub[-1], lb[-1], None, None, None, mask, lA, lb, ub, pre_relu_indices, slope_opt, history, None, A) if arguments.Config["cross_ex"]["enable_cross_ex"] else (ub[-1], lb[-1], None, None, None, mask, lA, lb, ub, pre_relu_indices, slope_opt, history, None)
        else:
            with torch.no_grad():
                if arguments.Config["cross_ex"]["enable_cross_ex"]:
                    lb, _, A = self.net.compute_bounds(
                            x=(x,), C=self.c, method=bounding_method, cutter=self.cutter, bound_upper=False, 
                            return_A= self.return_A, needed_A_dict = self.needed_A_dict, multiple_execution = self.multiple_execution, execution_count = self.execution_count, ptb = self.ptb, unperturbed_images = self.unperturbed_images, 
                            iteration = self.iteration, baseline_refined_bound = self.baseline_refined_bound, intermediate_bound_refinement = self.intermediate_bound_refinement,
                            always_correct_cross_execution = self.always_correct_cross_execution, cross_refinement_results = self.cross_refinement_results, populate_trace = self.populate_trace, target_lAs= target_lAs, target_biases = target_biases, topk=topk)
                else:
                ...
                lb, ub, pre_relu_indices = self.get_candidate(self.net, lb, lb + np.inf)
                return (ub[-1], lb[-1], None, None, None, None, None, lb, ub, pre_relu_indices, None, None, None, A) if arguments.Config["cross_ex"]["enable_cross_ex"] else (ub[-1], lb[-1], None, None, None, None, None, lb, ub, pre_relu_indices, None, None, None)
        ...
        return (ub[-1], lb[-1], mini_inp, duals, primals, mask, lA, lb, ub, pre_relu_indices, slope_opt, history, None, A) if arguments.Config["cross_ex"]["enable_cross_ex"] else (ub[-1], lb[-1], mini_inp, duals, primals, mask, lA, lb, ub, pre_relu_indices, slope_opt, history, None)
    ...
    def build_the_model_with_refined_bounds(self, input_domain, x, refined_lower_bounds, refined_upper_bounds,
            ...
            cutter=None, refined_betas=None, target_lAs= None, target_biases = None, topk=None):
        lr_init_alpha = arguments.Config["solver"]["alpha-crown"]["lr_alpha"]
        init_iteration = arguments.Config["solver"]["alpha-crown"]["iteration"]
        share_slopes = arguments.Config["solver"]["alpha-crown"]["share_slopes"]
        optimizer = arguments.Config["solver"]["beta-crown"]["optimizer"]
                ...
                if not self.return_A:
                    ...
                    if arguments.Config["cross_ex"]["enable_cross_ex"]:
                        ret = self.net.compute_bounds(
                            ...
                            C=self.c[now_batch * target_batch_size: (now_batch + 1) * target_batch_size],
                            reuse_alpha=True, intermediate_layer_bounds=intermediate_layer_bounds, 
                            return_A= self.return_A, needed_A_dict = self.needed_A_dict, multiple_execution = self.multiple_execution, execution_count = self.execution_count, ptb = self.ptb, unperturbed_images = self.unperturbed_images, 
                            iteration = self.iteration, baseline_refined_bound = self.baseline_refined_bound, intermediate_bound_refinement = self.intermediate_bound_refinement,
                            always_correct_cross_execution = self.always_correct_cross_execution, cross_refinement_results = self.cross_refinement_results, populate_trace = self.populate_trace, target_lAs= target_lAs, target_biases= target_biases, topk=topk)
                    else:
        ...
        return (ub[-1], lb[-1], mini_inp, duals, primals, mask, lA, lb, ub, pre_relu_indices, slope_opts, history, ret_b, A) if arguments.Config["cross_ex"]["enable_cross_ex"] else (ub[-1], lb[-1], mini_inp, duals, primals, mask, lA, lb, ub, pre_relu_indices, slope_opts, history, ret_b)


    def get_lower_bound_naive(
            self, dm_l=None, dm_u=None, slopes=None,
            bounding_method="crown", C=None, stop_criterion_func=None, target_lAs= None, topk=None):
        batch = len(dm_l)//2
        ...
        if arguments.Config["cross_ex"]["enable_cross_ex"]:
            self.needed_A_dict.get(self.net.output_name[0], set()).add(self.net.input_name[0])
        else:
        ...
        if bounding_method == "alpha-crown":
            if arguments.Config["cross_ex"]["enable_cross_ex"]:
                lb, _, A_dict = self.net.compute_bounds(
                    x=(new_x,), C=C, method= self.method, bound_upper=False, 
                return_A= self.return_A, needed_A_dict = self.needed_A_dict, multiple_execution = self.multiple_execution, execution_count = self.execution_count, ptb = ptb, unperturbed_images = self.unperturbed_images, 
                    iteration = self.iteration, baseline_refined_bound = self.baseline_refined_bound, intermediate_bound_refinement = self.intermediate_bound_refinement,
                    always_correct_cross_execution = self.always_correct_cross_execution, cross_refinement_results = self.cross_refinement_results, populate_trace = self.populate_trace, target_lAs= target_lAs, topk=topk)
            else:
            ...
            with torch.no_grad():
                if arguments.Config["cross_ex"]["enable_cross_ex"]:
                    lb, _, A_dict = self.net.compute_bounds(x=(new_x,), C=C, method=bounding_method,
                            bound_upper=False, return_A= self.return_A, needed_A_dict = self.needed_A_dict, multiple_execution = self.multiple_execution, execution_count = self.execution_count, ptb = ptb, unperturbed_images = self.unperturbed_images, 
                            iteration = self.iteration, baseline_refined_bound = self.baseline_refined_bound, intermediate_bound_refinement = self.intermediate_bound_refinement,
                            always_correct_cross_execution = self.always_correct_cross_execution, cross_refinement_results = self.cross_refinement_results, populate_trace = self.populate_trace, target_lAs= target_lAs, topk=topk)
                else:
                    lb, _, A_dict = self.net.compute_bounds(x=(new_x,), C=C, method=bounding_method,
        ...
        return (lb, [None] * (batch * 2), ret_s, lA, A_dict) if arguments.Config["cross_ex"]["enable_cross_ex"] else (lb, [None] * (batch * 2), ret_s, lA)
    ...
    from lp_mip_solver import (build_solver_model, update_mip_model_fix_relu,
                            ...
                            all_node_split_LP)
    from bab_bounds_multiple import ( update_bounds_parallel_multiple, prepare_intermediate_layer_bounds, compute_splits_per_img, concat_beta_multiple, concat_slopes_multiple)
    from bab_bounds_targetted import (update_bounds_parallel_targetted, reset_targetted_beta, set_beta_sign_loc, expand_intermediate_layer_bounds)
