...
@torch.no_grad()
def compute_babsr_score(lower_bounds, upper_bounds, orig_mask, net, pre_relu_indices, lAs, sparsest_layer=0,
...
def choose_node_parallel_crown(lower_bounds, upper_bounds, orig_mask, net, pre_relu_indices, lAs, sparsest_layer=0,
                               decision_threshold=0.001, batch=5, branching_reduceop='min', split_depth=1, cs=None, rhs=0, score = None, intercept_tb = None):
    batch = min(batch, len(orig_mask[0]))
    # Mask is 1 for unstable neurons. Otherwise it's 0.
    mask = orig_mask

    if score is None or intercept_tb is None:
        score, intercept_tb = compute_babsr_score(lower_bounds, upper_bounds, orig_mask, net, pre_relu_indices, lAs, sparsest_layer, decision_threshold, batch, branching_reduceop, split_depth, cs, rhs)
...
def indices_of_largest_scores_per_group(scores, group_ids, group_idx = None):
    max_indices = {}
    max_scores = {}

    for i, (score, group_id) in enumerate(zip(scores, group_ids)):
        if group_id not in max_scores or score[0] > max_scores[group_id]:
            max_scores[group_id] = score[0]
            max_indices[group_id] = i
        
    if group_idx is None:
        return list(max_indices.values())
    else:
        return max_indices[group_id], max_scores[group_id]

@torch.no_grad()
def choose_node_parallel_multiple_kFSB(lower_bounds, upper_bounds, orig_mask, net, pre_relu_indices, lAs,
                              ...
                              split_depth=1, cs=None, rhs=0, method='kfsb', group_ids = None):
    
    batch = [len(orig_mask[i][0]) for i in range(len(orig_mask))]
    # Mask is 1 for unstable neurons. Otherwise it's 0.
    mask = orig_mask
    reduce_op = get_branching_op(branching_reduceop)
    # FIXME: it seems cs and should always be not None because they are used below.
    number_bounds = [1] * len(cs) if cs is None else [c.shape[1] for c in cs]
    
    scores = []
    for i in range(len(lower_bounds)):
        if orig_mask[i] is not None:
            score, intercept_tb = branching_scores_kfsb(lower_bounds[i], upper_bounds[i], net, pre_relu_indices,
                                                    lAs[i], batch[i], mask[i], reduce_op, number_bounds[i], prioritize_slopes)
            scores.append((max([max([s.max().item() for s in score[j]]) for j in range(len(score))]), score, intercept_tb))
        else:
            #if mask is None or skip idx (img is verified) don't branch
            scores.append((0, None, None))
    
    idxs_to_branch = indices_of_largest_scores_per_group(scores, group_ids)
    final_decisions, split_depths = [], []
    
    for group_id, idx_to_branch in enumerate(idxs_to_branch):
        score, intercept_tb = scores[idx_to_branch][1], scores[idx_to_branch][2]
        if score is None:
            return None, None, -1
        
        final_decision, sd = compute_multi_kFSB(lower_bounds = lower_bounds, upper_bounds= upper_bounds, 
                                                       orig_mask = orig_mask[idx_to_branch], net = net, pre_relu_indices = pre_relu_indices, lAs = lAs, 
                                                       branching_candidates= branching_candidates, branching_reduceop= branching_reduceop, 
                                                       slopes = slopes, betas = betas, history = history, 
                                                       use_beta = use_beta, keep_all_decision= keep_all_decision, prioritize_slopes= prioritize_slopes, 
                                                       split_depth= split_depth[idx_to_branch], cs = cs, rhs = rhs, precomputed_score= (score, intercept_tb), 
                                                       idx_to_branch= idx_to_branch)
        
        if len(final_decision) < len(orig_mask[idx_to_branch][0]):
            #all nodes visited. Let's try another image
            print(f'all nodes are split!! for image {idx_to_branch}')
            max_score = float('inf')
            while (len(final_decision) < len(orig_mask[idx_to_branch][0])):
                if max_score <= 0:
                    return None, None, -1
                
                scores[idx_to_branch] = (0, None, None)
                idx_to_branch, max_score = indices_of_largest_scores_per_group(scores, group_ids, group_id)
                score, intercept_tb = scores[idx_to_branch][1], scores[idx_to_branch][2]
                final_decision, sd = compute_multi_kFSB(lower_bounds = lower_bounds, upper_bounds= upper_bounds, 
                                                       orig_mask = orig_mask[idx_to_branch], net = net, pre_relu_indices = pre_relu_indices, lAs = lAs, 
                                                       branching_candidates= branching_candidates, branching_reduceop= branching_reduceop, 
        ...
        split_depths.append(sd)           
    return final_decisions, split_depths, idxs_to_branch
    
@torch.no_grad()
def choose_node_parallel_multiple(lower_bounds, upper_bounds, orig_mask, net, pre_relu_indices, lAs, sparsest_layer=0,
                        decision_threshold=0.001, batch=5, branching_reduceop='min', split_depth=1, cs=None, rhs=0, group_ids = None):
    scores = []
    for i in range(len(lower_bounds)):
        if orig_mask[i] is not None:
            score, intercept_tb = compute_babsr_score(lower_bounds[i], upper_bounds[i], orig_mask[i], net, pre_relu_indices, lAs[i], batch=batch, branching_reduceop=branching_reduceop, split_depth=split_depth[i], cs=cs[i], rhs=rhs[i])
            scores.append((max([max([s.max().item() for s in score[j]]) for j in range(len(score))]), score, intercept_tb))
        else:
            #if mask is None don't branch
            scores.append((0, None, None))
    ...
    for group_id, idx_to_branch in enumerate(idxs_to_branch):
        ...
        final_decision, sd = choose_node_parallel_crown(lower_bounds[idx_to_branch], upper_bounds[idx_to_branch], orig_mask[idx_to_branch], net, pre_relu_indices, lAs[idx_to_branch], batch=batch, branching_reduceop=branching_reduceop, split_depth=split_depth[idx_to_branch], cs=cs[idx_to_branch], rhs=rhs[idx_to_branch], score = score, intercept_tb= intercept_tb)
...
@torch.no_grad()
def choose_node_parallel_targetted(lower_bounds, upper_bounds, orig_mask, net, pre_relu_indices, lAs, sparsest_layer=0,
                        decision_threshold=0.001, batch=5, branching_reduceop='min', split_depth=1, cs=None, rhs=0):
    scores = []
    for i in range(len(lower_bounds)):
        if orig_mask[i] is not None:
    ...
    idx_to_branch = max(range(len(scores)), key=lambda i: scores[i][0])
    score, intercept_tb = scores[idx_to_branch][1], scores[idx_to_branch][2]
    ...
    if len(final_decision) < len(orig_mask[idx_to_branch][0]):
        ...
        print(f'all nodes are split!! for image {idx_to_branch}')
        while (len(final_decision) < len(orig_mask[idx_to_branch][0])):
            if max(scores, key = lambda x : x[0])[0] <= 0:
                return None, None, -1
    ...
    return final_decision, sd, idx_to_branch

@torch.no_grad()
def compute_multi_kFSB(lower_bounds, upper_bounds, orig_mask, net, pre_relu_indices, lAs,
                              branching_candidates=5, branching_reduceop='min', slopes=None,
                              betas=None, history=None, use_beta=False, keep_all_decision=False, prioritize_slopes='none',
                              split_depth=1, cs=None, rhs=0, method='kfsb', precomputed_score = None, idx_to_branch = None):

    assert (method == 'kfsb') and (idx_to_branch is not None)
    batch = len(orig_mask[0])
    # Mask is 1 for unstable neurons. Otherwise it's 0.
    mask = orig_mask
    reduce_op = get_branching_op(branching_reduceop)
    ...
    if precomputed_score is not None:
        score, intercept_tb = precomputed_score
    else:
        raise ValueError("score must be pre-computed for now")
    ...
    if method == 'kfsb-intercept-only':
        ...
        # real batch = batch * 2, since we have two kinds of scores.
        lbs = [[torch.cat([i, i]) for i in lower_bnds] for lower_bnds in lower_bounds]
        ups = [[torch.cat([i, i]) for i in upper_bnds] for upper_bnds in upper_bounds]
        
        sps = []
        for slps in slopes:
            if isinstance(slps, dict):
                # per neuron slope.
                _sps = defaultdict(dict)
                for k, vv in slps.items():
                    _sps[k] = {}
                    for kk, v in vv.items():
                        _sps[k][kk] = torch.cat([v, v], dim=2)
            else:
                _sps = [torch.cat([i, i]) for i in slps]
            sps.append(_sps)
        
        if use_beta:
            bs = [[torch.cat([i, i]) for i in bts] for bts in betas]
            for hist in history:
                hist += hist
        
        css = [torch.cat([c_matrx, c_matrx]) for c_matrx in cs] 
        rhs = [torch.cat([r_matrx, r_matrx]) for r_matrx in rhs]
    ...
    k_ret = torch.empty(size=(topk, batch_size), device=lower_bounds[idx_to_branch][0].device, requires_grad=False)
    set_slope = True  # We only set the slope once.
    for k in range(topk):
        ...
        # only save the best lower bounds of the two splits
        if use_beta:
            k_ret_lbs = net.update_bounds_parallel_multiple(all_img_pre_lb = lbs, all_img_pre_ub = ups, all_splits = {idx_to_branch: k_decision[-1]}, 
                                                   all_img_slopes = sps if set_slope else [[] for _ in range(len(lbs))], all_img_betas =bs,
                                                   fix_intermediate_layer_bounds=True, shortcut=True, all_img_history = history, all_img_cs=css, 
                                                   branch_idxs = [idx_to_branch])
        else:
            k_ret_lbs = net.update_bounds_parallel_multiple(all_img_pre_lb = lbs, all_img_pre_ub = ups, all_splits = {idx_to_branch: k_decision[-1]}, 
                                                   all_img_slopes = sps if set_slope else [[] for _ in range(len(lbs))], beta=False,
                                                   fix_intermediate_layer_bounds=True, shortcut=True, all_img_cs=css, 
                                                   all_img_history = [[] for _ in range(len(lbs))], branch_idxs = [idx_to_branch])
        # print(f'k {k} decision {k_decision[-1]} bounds {k_ret_lbs.squeeze(-1).cpu()}')
        # consider the max improvement among multi bounds in one C matrix
        k_ret_lbs = (k_ret_lbs - torch.cat([rhs[idx_to_branch], rhs[idx_to_branch]])).max(-1).values
        # No need to set slope next time; we do not optimize the slopes.
        set_slope = False
